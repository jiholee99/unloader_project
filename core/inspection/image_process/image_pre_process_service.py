...
class ImagePreprocessService:
    ...
    def __init__(self, preProcessor : ImagePreprocessor, config: dict = {}, roi_coords: list[int] = []):
    ...
    def process_image(self, image, threshold: int = 0, max_threshold = 255):
        """
        ...
        try:
            ...
            self._roi_image = self.preprocessor.crop_roi(image, *self._roi_coords,)
            preprocessed_mask_img = self._roi_image
            # Optional: Remove bright line if enabled
            if self._config.get("use_remove_bright_line", False):
            ...
            mask = self.preprocessor.apply_threshold(gray, self._config.get("threshold", threshold), max_threshold)
            return mask
