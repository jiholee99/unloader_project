class SignalStore:
    ...
    def set_signals(self, signals: dict):
        print(f"[DEBUG] set_signals called: {signals}")
        """Store full dict from master."""
        self._signals.update(signals) 

    def get_signals(self):
        """Return dict for slaves."""
        print(f"[DEBUG] SignalStore current state: {self._signals}")
    ...
    def reset_signal(self, slave_id):
        ...
        if slave_id in self._signals:
            ...
            print(f"[DEBUG] Reset {slave_id} → False")
        else:
            print(f"[DEBUG] reset_signal ignored — {slave_id} not in signals")
